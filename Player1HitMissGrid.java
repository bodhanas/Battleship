 ...
 * @author Bodhana S.
 * @date June 12, 2022
 * Hit and Miss Grid through which PLAYER 1 can "attack" or guess where Player 2's ships are located
 */
...
public class Player1HitMissGrid extends javax.swing.JFrame {

    JButton locationGrid[][] = PlayerReferences.player2inst.returnPlayer2Grid(); //Initialize location grid of Player 1 and update buttons based on hits
    char p2Ships[][] = PlayerReferences.player2inst.returnPlayer2Ships(); //Initialize array of chars to store location of Player 2's ships
    JButton hitMissGrid[][] = new JButton[10][10]; //Initialize array of buttons to display and update Player 1's Hit Miss Grid
    
    /**
     * Places GUI buttons in array, which is updated based on the buttons user selects
     * Player 1's Fleet Location Grid is also displayed and will be updated based on "hits" by Player 2
     */
    public Player1HitMissGrid() {
        initComponents();
    ...
    /**
     * Refer to unClick.Player1FleetLocationGrid for parameters
     * Checks whether button selected by Player 1 is a "hit" or a "miss"
     */
    private void hitOrMiss(int r, int c) {
        int numShipsLeft = 1;//Initialize ships remaining to "hit"; Assigned value of 1 to ensure loop runs first time through
        if (p2Ships[r][c] == '-') { //Runs if square is open
            hitMissGrid[r][c].setBackground(new Color(255, 252, 187)); //Changes background color to yellow in hit miss grid to signify a "miss"
            p2Ships[r][c] = 'X'; //Char assigned "X" to signify that it has been selected
            JOptionPane.showMessageDialog(null, "That was a MISS!"); //Display informational message to Player 1
            numShipsLeft = checkWinner(); //Checks if Player 1 has won
            if (numShipsLeft != 0) { //Runs if there are still ships remaining from Player 2's fleet
                nextTurn(); //Initiates Player 2's turn
            }
        } else if (p2Ships[r][c] != '-' && p2Ships[r][c] != 'X') { //Runs if square is not available OR has been selected; Checks if specific char of ship is present
            hitMissGrid[r][c].setBackground(new Color(255, 127, 127)); //Changes background color to red in hit miss grid to signify a "hit"
            //Refer to previous loop for comments
            p2Ships[r][c] = 'X';
            locationGrid[r][c].setBackground(new Color(97, 105, 110)); //Changes background color to dark grey in location grid to signify a "hit"
    ...
    /**
     * Initiates next turn by closing Player 1's grids and opening Player 2's ones
     */
    private void nextTurn() {
        JOptionPane.showMessageDialog(null, "REMINDER: Turn around, so Player 2 can attack."); //Reminder to Player 1 to turn around during Player 2's turns
        //Refer WelcomePage.playBtnActionPerformed comments
        //Close Player 1's Hit Miss Grid and Fleet Location Grid
        //Displays Player 2's Hit Miss Grid and Fleet Location Grid
        if (PlayerReferences.player1HitMissinst == null) {
            PlayerReferences.player1HitMissinst = new Player1HitMissGrid();
        }
        PlayerReferences.player1HitMissinst.setVisible(false);
        if (PlayerReferences.player2HitMissinst == null) {
            PlayerReferences.player2HitMissinst = new Player2HitMissGrid();
        }
        PlayerReferences.player2HitMissinst.setVisible(true);
    ...
    private EndPage gameOver; //Initializes instance of final window that displays winner

    /**
     * Checks if Player 1 won game
     * @return int - # of ships left
     */
    private int checkWinner() {
        int numShipsLeft = 0; //Initializes accumulator to count number of ships Player 2 has remaining after "hits"
        for (int r = 0; r < 10; r++) {
            for (int c = 0; c < 10; c++) {
                if (p2Ships[r][c] != '-' && p2Ships[r][c] != 'X') {
                    numShipsLeft = numShipsLeft + 1; //Increment counter by 1 for each unique ship char (hence not hit yet)
                }
            }
        }
        if (numShipsLeft == 0) { //Runs if Player 2 has no ships left and hence, Player 1 is the winner!
            //Refer WelcomePage.playBtnActionPerformed comments
            //Close Player 1's Fleet Location Grid and Hit and Miss Grid, since game is over
            if (PlayerReferences.player1HitMissinst == null) {
                PlayerReferences.player1HitMissinst = new Player1HitMissGrid();
            }
            PlayerReferences.player1HitMissinst.setVisible(false);
            ...
            PlayerReferences.player1inst.setVisible(false);
            //Display final window that displays the winner, Player 1
            if (gameOver == null) {
                gameOver = new EndPage("Player 1");
            }
    ...
    /**
     * Determines whether button is available to hit or miss
     * If YES, check whether ship is present and updates array storing data of which squares are selected
     */
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Player1HitMissGrid().setVisible(true);
            }
        });
    }
