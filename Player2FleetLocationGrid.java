 ...
 * @author Bodhana S.
 * @date June 12, 2022
 * Fleet Location Grid that stores and displays location of PLAYER 2's ships
 */
...
/**
 * Refer to Player1FleetLocationGrid class for comments
 */
public class Player2FleetLocationGrid extends javax.swing.JFrame {
    
    JButton grid[][] = new JButton[10][10];
    char ships[][] = new char[10][10];
    int counter = 0;

    public Player2FleetLocationGrid() {
        initComponents();
        grid[0][0] = btn00;
        grid[0][1] = btn01;
        grid[0][2] = btn02;
    ...
    private void chooseTopRow(int r, int c, char ship, Color name) {
        if (ships[r + 1][c] == ship || ships[r][c + 1] == ship || ships[r][c - 1] == ship) {
            chooseSquare(r, c, ship, name);
        }
    }
    ...
    private void chooseLeftCol(int r, int c, char ship, Color name) {
        if (ships[r + 1][c] == ship || ships[r - 1][c] == ship || ships[r][c + 1] == ship) {
            chooseSquare(r, c, ship, name);
        }
    }
    ...
    private void chooseInterior(int r, int c, char ship, Color name) {
        if (ships[r + 1][c] == ship || ships[r - 1][c] == ship || ships[r][c + 1] == ship || ships[r][c - 1] == ship) {
            chooseSquare(r, c, ship, name);
        }
    }

    public static boolean isAllEqual(int[] e) {
    ...
    private void choose2ndSquare(int r, int c, char ship, Color name) {
        if (r == 0 && c == 0) {
            chooseTopLeftCorner(r, c, ship, name);
        } else if (r == 9 && c == 0) {
    ...
    private void chooseRemainingSquares(int r, int c, char ship, Color name) {
        ...
        int y[] = new int[0];
        boolean direction;
        direction = findDirection(x, y, ship);
        if (direction == false) {
            if (r == 0) {
                if (ships[r + 1][c] == ship) {
                    chooseSquare(r, c, ship, name);
                ...
                if (ships[r + 1][c] == ship || ships[r - 1][c] == ship) {
                    chooseSquare(r, c, ship, name);
                }
            }
    ...
    private void carrier(int r, int c) {
        Color purple = new Color(207, 159, 255);
        char ship = 'C';
        if (ships[r][c] != ship) {
            if (counter == 0) {
    ...
    private void chooseBoard(int r, int c) {
        if (counter < 5) {
            ...
            destroyer(r, c);
            if (ships[r][c] == 'D') {
                JOptionPane.showMessageDialog(null, "Player 2's board is set. REMINDER: Turn around, so Player 1 can start. May the best fleet win...");
                playerNum.setText(null);
                locationText.setText(null);
                shipName.setText("Player 2's Fleet");
                shipName.setForeground(Color.BLACK);
                //Display Player 1's Hit Miss Grid and close Player 2's Fleet Location Grid
    ...
    public JButton[][] returnPlayer2Grid() {
        return grid.clone();
    }

    public char[][] returnPlayer2Ships() {
        return ships.clone();
    }
    ...
    public void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Player2FleetLocationGrid().setVisible(true);
            }
        });
    }
